...
class KSPayWebHost
{
    ...
    public function sendMsg($_mtype)
    {
        $this->mtype = $_mtype;
        $rmsg = $this->send_url();
    ...
    {
        // PC, Mobile용 URI 구분
        $this->KSPAY_WEBHOST_URI = \Browser::isDesktop() ? "/store/KSPayFlashV1.3/web_host/recv_post.jsp" : "/store/mb2/web_host/recv_post.jsp";
        ...
        $kspay_port   = 80;
        $this->write_log("send_url:send(" . $this->payKey . ",".$kspay_ipaddr.",".$kspay_port.")=[".$post_msg."]");
        if ($kspay_ipaddr == $this->KSPAY_WEBHOST_HOST)
        {
            $kspay_ipaddr = $this->KSPAY_WEBHOST_IP;
            $this->write_log("CHECK: gethostbyname(" . $this->KSPAY_WEBHOST_HOST . "):X DEFALUT IP=[".$this->KSPAY_WEBHOST_IP."]");
        }
        ...
        $this->write_log("send_url:recv(" . $this->payKey . ",".$kspay_ipaddr.",".$kspay_port.")=[".$rtn_msg."]");
